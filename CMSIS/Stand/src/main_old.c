...
// Структура описания аргумента инструкции
struct argument {
    const char* name;               // Имя аргумента
    const char* description;        // Описание аргумента
    const uint8_t link_count: 7;    // Количество константных значений (ссылок) аргумента
    const uint8_t hasDefault: 1;    // Имеет ли значение по умолчанию
};

// Структура описания константного значения (ссылки) аргумента инструкции
...
// Список всех аргументов инструкций
const struct argument arg_list[] = {   
    {"pid-d", "to get pid-d", 0, 1}, 
    {"pid-p", "to get pid-p", 0, 1},
    {"pid-i", "to set pid-i", 0, 1},
    {"pid-d", "to set pid-d", 1, 1}
};

// Список всех константных значений (ссылок) всех аргументов инструкций
const struct argument_value link_list[] = {   
    { "digital", "to use digital value" }
};

// Строка с именем значения по умолчанию
const char* link_default_name = "default";
// Строка с описанием значения по умолчанию
const char* link_default_description = "to use default value of the argument";

#define instr(index) (instr_list + index)
#define instr_list_count sizeof(instr_list) / sizeof(struct instruction)
#define arg_list_count sizeof(arg_list) / sizeof(struct argument)

const struct argument* arg(__uint8_t instr_index, __uint8_t arg_index) {
    for (uint8_t i = 0; i < instr_index; i++) arg_index += instr_list[i].arg_count;
    return arg_list + arg_index;
}

const struct argument_value* link(__uint8_t instr_index, __uint8_t arg_index, __uint8_t link_index) {
    for (uint8_t i = 0; i < instr_index; i++) arg_index += instr_list[i].arg_count;
    // Если данный аргумент имеет значение по умолчанию
    if (arg_list[arg_index].hasDefault) {
        
    } 
    for (uint8_t i = 0; i < arg_index; i++) link_index += arg_list[i].link_count;
    return link_list + link_index;
}
